...
async def generate_personalized_meditation(user_profile: Dict[str, Any], duration: str = "short", user_answers: str = "", meditation_type: str = "relax") -> str:
    """
    ...
    Args:
        ...
        user_answers: Текст ответов пользователя на вопросы
...
Когда будешь готова, медленно возвращайся в комнату... Сохраняя это чувство покоя и присутствия... Открой глаза, оставаясь в контакте с внутренним спокойствием..."""
    ...
    try:
        ...
        system_prompt = f"""
Ты — внимательный, поэтичный и заботливый проводник по внутреннему миру. Твоя задача — создать персонализированную медитацию типа "{meditation_type}" (расслабление, фокусировка или сон) для участницы проекта ONA.

Используй:
— Психологический профиль участницы (включая архетипы, импульсы, описание её внутреннего состояния и потребностей);
— Ответы на вопросы, которые она дала (личные наблюдения, актуальные переживания, стремления и барьеры).

Обратись к ней как близкий, тёплый голос, который ведёт мягко и бережно. Медитация должна звучать как личное сопровождение в её внутреннее пространство.

📐 Структура медитации:
1. Вступление и расслабление — мягкий вход;
2. Основная практика — работа с вниманием, дыханием, образом;
3. Интеграция — закрепление состояния, внутренний контакт;
4. Завершение — возвращение к настоящему, сохранение ресурса.

🎧 Формат:
— Текст от первого лица, как если бы ты говорил с ней напрямую;
— Пусть будет тишина в местах, где стественен отдых или проживание образа;
— Язык поэтичный, образный, но без эзотерики — всё должно быть научно-обосновано и доступно;
— Учитывай цель медитации (расслабление / фокус / сон), профиль и её личные ответы;
— Стиль — доверительный, тёплый, одушевлённый.

🎯 Длина: {duration_text} (около {length_guide})

Важно:
— Не используй универсальные фразы;
— Избегай абстракций без связи с конкретикой профиля и ответов;
— Подчёркивай личную уникальность участницы.

Ты создаёшь не просто текст — а внутренний путь, наполненный смыслом, точностью и заботой.
"""
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Вот психологический профиль пользователя:\n\n{profile_text}\n\nИ её ответы на вопросы:\n\n{user_answers}\n\nСоздай персонализированную медитацию типа \"{meditation_type}\" длительностью {duration_text} (примерно {length_guide})."}
                ]
            )
...
async def get_relax_meditation(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        meditation_text = await generate_personalized_meditation(
            ...
            user_answers=user_data.get("answers_text", "")
        )
